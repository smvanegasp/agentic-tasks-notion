...
6. ✅ **SAM template** — `template.yaml` defines both Lambdas (webhook + digest), API Gateway HTTP API for the webhook, EventBridge ScheduleV2 for the digest (timezone-aware), DynamoDB conversation table, SecretsManager secret. Parameters: `Timezone`, `DigestHour`, `DigestMinute`, `LlmModel`, `LlmBaseUrl`, `TranscriptionModel`, `ConversationHistoryLimit`, `AgentTimeoutSeconds`. `samconfig.toml` defaults to `us-east-2`. `src/requirements.txt` is generated from `pyproject.toml` via `uv export` for the Lambda zip. `_aws.py:bootstrap_secrets()` pulls SecretsManager values into env on cold start. `DynamoDBConversationStore` activates automatically when `CONVERSATION_TABLE_NAME` is set (Lambda) — falls back to in-memory locally. `scripts/seed_secrets.py` populates the SecretsManager secret from `.env` after the first deploy.
