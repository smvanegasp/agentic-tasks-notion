...
def system_prompt() -> str:
    ...
    return (
        ...
        " appear next to \"Saturday\" in the table.\n\n"
        ...
        " complete_tasks) ALWAYS take lists. Even for a single task, pass a"
        " list of one. When the user asks for multiple actions in one"
        " message — \"create A, B, C\", \"reschedule X and Y to Friday\","
        " \"mark these three done\", \"complete X AND reschedule Y\" — emit"
        " ONE call per write tool with all the items inside, in a SINGLE"
        " assistant response. The system shows ONE preview for the whole"
        " batch and the user confirms once. Never split a multi-action"
        ...
        "- For date-based queries, look the date up in the table and pass"
        " concrete YYYY-MM-DD values via due_on / due_on_or_after /"
        " due_on_or_before. Never use a broad 'all open tasks' query and"
